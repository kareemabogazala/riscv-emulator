...
{
    return (addr >= MemoryLayout::RAM_BASE &&
            addr < MemoryLayout::RAM_BASE + MemoryLayout::RAM_SIZE);
}
...
uint8_t MemoryBus::read8(uint32_t addr) const
{
    ...
    throw std::out_of_range("MemoryBus::read8: address " + fmt_addr(addr) + " not mapped");
}
...
uint16_t MemoryBus::read16(uint32_t addr) const
{
    ...
    throw std::out_of_range("MemoryBus::read16: address " + fmt_addr(addr) + " not mapped");
}
...
uint32_t MemoryBus::read32(uint32_t addr) const
{
    ...
    throw std::out_of_range("MemoryBus::read32: address " + fmt_addr(addr) + " not mapped");
}
...
void MemoryBus::write8(uint32_t addr, uint8_t val)
{
    ...
    throw std::out_of_range("MemoryBus::write8: address " + fmt_addr(addr) + " not in data region");
}
...
void MemoryBus::write16(uint32_t addr, uint16_t val)
{
    if (in_range(addr) && in_range(addr + 1))
    {
        ram.write16(addr, val);
        return;
    }
    throw std::out_of_range("MemoryBus::write16: address " + fmt_addr(addr) + " not in data region");
}
...
void MemoryBus::write32(uint32_t addr, uint32_t val)
{
    ...
    throw std::out_of_range("MemoryBus::write32: address " + fmt_addr(addr) + " not in data region");
}
